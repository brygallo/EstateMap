...
- `backend/real_estate/models.py:7-16` (`class User(AbstractUser)`) — Definición completa del usuario. Los únicos campos propios son email, is_email_verified y los tres de OAuth.
...
- `backend/estate_map/settings.py:373-375` (`MAX_IMAGES_PER_PROPERTY`) — La línea 367 es el comentario "User-specific limits can be introduced here when subscription plans are implemented".
...
- `backend/estate_map/settings.py:187-196` (`'property_write': '30/hour'`) — Los cinco scopes de throttling. Ninguno cuenta propiedades existentes; todos miden peticiones por ventana de tiempo.
