# Control de calidad independiente — video-010

## PASS — reverificación del 2026-08-14 (19:16)

CONTRACT: VIDEO_COUNCIL_V1 · rol 5 · **reverificación acotada del expediente**. No se
volvió a abrir el máster: `exports/video.mp4` conserva su mtime `19:01:56`, el mismo que
...
|---|---|---|---|
| 1 | `python3 factory.py docs video-010` | **verificado** | `script.md`, `storyboard.md` y `caption.txt` con mtime `19:15:31`, posterior a `plan.json` (`18:52:53`). `caption.txt` es idéntico carácter a carácter a `plan.json:14` |
...
| 3 | `.render.lock` ignorado | **verificado** | `/Users/usuario/gad/EstateMap/.gitignore:78` → `marketing/videos/library/**/.render.lock`; `git status --porcelain library/video-010/` ya no lo lista |
...
Reejecutados por mí, no leídos del archivo: `python3 factory.py lint video-010` → `lint OK ·
...
derivado**; siguen en el catálogo de `renderer.py` porque las usa el video-009, que está
...
| R5 | La serie no se llama igual en el catálogo | **abierta** | `memory/catalog.json`: video-009 `"Antes de comprar un terreno"`, video-010 `"Antes de comprar"` |
...
CONTRACT: VIDEO_COUNCIL_V1 · rol 5 · revisión del máster `exports/video.mp4` (83,179 s,
...
**Corrección:** `python3 factory.py docs video-010`, que reescribe `script.md`,
...
**R5 · La serie no se llama igual en el catálogo.** `memory/catalog.json`: video-009 declara
`"series": "Antes de comprar un terreno"` y video-010 `"Antes de comprar"`. Dueño: editor jefe.
...
fotograma 0,20 s), y video-009 está `signed` con la misma estructura, así que no lo trato como
...
**R9 · Queda `library/video-010/.render.lock` sin seguimiento en git.** Es inofensivo —el
...
- **Comandos.** `python3 factory.py lint video-010` → `lint OK · 0 errores, 0 avisos ·
...
- Vuelve al editor jefe. Un `python3 factory.py docs video-010`, una relectura de los dos
