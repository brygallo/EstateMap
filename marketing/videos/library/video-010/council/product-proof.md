...
Pieza: `library/video-010` (renderizada, `review.json` `passed: true`, sin firma humana).
...
6. **El precio por metro cuadrado es una división real que el portal ya hace.**
   ...
   - Eso respalda **el método**. Las cifras concretas de la escena 6 no necesitan fuente:
     son dato de ejemplo permitido por `marketing/videos/memory/decisions.md:51` y
...
1. **La regla no prohíbe una cifra de ejemplo.** `marketing/videos/memory/decisions.md:48-51`,
